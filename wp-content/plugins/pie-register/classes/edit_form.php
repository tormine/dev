...
class Edit_form extends PieReg_Base
{
	...
	function addPassword()
	{
		...
		$data .= '<input id="'.$this->id.'" name="password" class="'.$this->addClass("input_fields",array("minSize[8]")).'" placeholder="'.$this->field['placeholder'].'" type="password" value="" autocomplete="off" />';	
			...
			$data .= '</div></li><li class="fields pageFields_'.$this->pages.' '.$topclass.'"><div class="fieldset"><label>'.__("Confirm Password","piereg").'</label><div '.$fclass.'><input id="confirm_password_'.$this->id.'" type="password" class="input_fields validate[required,equals['.$this->id.']]" placeholder="'.$this->field['placeholder'].'" value="" name="confirm_password" autocomplete="off">';
		return $data;	
	}	
	function addEmail()
	{
	...
	{		
		$val = get_usermeta($this->user->data->ID , $this->slug);
		return '<input id="'.$this->id.'" class="'.$this->addClass().'" name="'.$this->name.'" class="input_fields"  placeholder="'.$field['placeholder'].'" type="text" value="'.$val.'" />';	
	}
	function addList()
	{
		$data = "";
		...
		for($a = 1 ,$c=0; $a <= $this->field['rows'] ; $a++,$c++)
		{
				...
				{
					$data .= '<input style="width:'.$width.'%;margin-right:2px;padding:0px;" type="text" name="'.$this->name.'['.$c.'][]" class="input_fields" value="'.$val[$c][$b-1].'"> ';
					$row 	.= $value[$a][$b-1];
				}	
		...
		if(sizeof($this->field['value']) > 0)
		{
			...
			for($a = 0 ; $a < sizeof($this->field['value']) ; $a++)
			{
				...
				{	
					...
					$data .= "</label>";
					$data .= '<input '.$checked.' value="'.$this->field['value'][$a].'" type="'.$this->field['type'].'" '.$multiple.' name="'.$this->name.'[]" class="'.$this->addClass("").' radio_fields" >';
	...
	function addSubmit()
	{
		...
		//else
		//{
			$data .= '<input name="pie_submit_update" type="submit" value="'.$this->field['text'].'" />';	
		//}
		return $data;
	}
	...
	function validateRegistration($errors)
	{
		...
		if ( empty( $_POST['e_mail'] ) || !filter_var($_POST['e_mail'],FILTER_VALIDATE_EMAIL) )
		{
		 ...
		 {
			$errors->add( $slug , '<strong>'.__(ucwords('Error'),'piereg').'</strong>: '.apply_filters("piereg_password_Fields_do_not_macth",__('Password Fields do not macth!','piereg' )));
		 }	
		 ...
		 foreach($this->data as $field)
		 {
			...
			else if($rule=="alphanumeric")
			{
				...
				{
					$errors->add( $slug , '<strong>'.__(ucwords('Error'),'piereg').'</strong>: '.$field['label'] .apply_filters("piereg_field_may_only_contain_alpha_numeric_characters",__(' field may only contain alpha-numeric characters.','piereg' )));
				}	
			}	
			else if($rule=="email")
			{
				...
				{
					$errors->add( $slug , '<strong>'.__(ucwords('Error'),'piereg').'</strong>: '.$field['label'] .apply_filters("piereg_field_must_contain_a_valid_email_address",__(' field must contain a valid email address.','piereg' )));	
				}	
			}	
