...
function ipin_user_register($user_id) {
	...
	$add_default_boards = array(
		...
		foreach ($add_default_boards as $add_default_board => $default_cat ) {
		$slug = wp_unique_term_slug(str_replace(" ", "-", strtolower($add_default_board)) . '__ipinboard', 'board');
		
		$result = wp_insert_term ( $add_default_board, 'board',
				array(
					'description' => $default_cat,
...
//Check and add parent board upon login (in case user did not register through  theme register page
...
//check just in case to create default corresponding boards with categories for the the default categories (in case user did not register through register page
