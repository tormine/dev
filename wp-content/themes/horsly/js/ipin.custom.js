jQuery(document).ready(function($) {
					...
					success: function(count) {
						...
						if (count == 1) {
							if ($('#post-repins').length) {
								$('#post-repins').before('<div class="post-likes"><div class="post-likes-wrapper"><h4>' + obj_ipin.__Likes + '</h4><div class="post-likes-avatar"><a href="' + obj_ipin.home_url + '/user/' + obj_ipin.ul + '/" rel="tooltip" title="' + obj_ipin.ui + '">' + obj_ipin.avatar48 + '</a></div></div></div>');
							} else {
								$('#post-embed-overlay').before('<div class="post-likes"><div class="post-likes-wrapper"><h4>' + obj_ipin.__Likes + '</h4><div class="post-likes-avatar"><a href="' + obj_ipin.home_url + '/user/' + obj_ipin.ul + '/" rel="tooltip" title="' + obj_ipin.ui + '">' + obj_ipin.avatar48 + '</a></div></div></div>');
							}
							...
							$('.post-likes-avatar').append('<a id=likes-' + obj_ipin.u +  ' href="' + obj_ipin.home_url + '/user/' + obj_ipin.ul + '/" rel="tooltip" title="' + obj_ipin.ui + '">' + obj_ipin.avatar48 + '</a>');
	...
	$(document).on('click', '.ipin-repin', function() {
		if (obj_ipin.u != '0') {
			...
			var post_description = $('#masonry #post-' + post_id + ' .post-title').text();
			...
			if ($('#newboard').length) {
				...
				$.ajax({
					...
					success: function(data) {
						...
						//when in single.php
						if (post_description == '') {
							post_description = $('#post-' + post_id + ' .post-title').text();
						}
	...
	$(document).on('submit', '#repinform', function() {
		...
		repin_status.html('');
		repin_status.html(' <img src="' + obj_ipin.stylesheet_directory_uri + '/img/ajax-loader-2.gif" />');
		...
		$.ajax({
			...
			success: function(data) {
				...
				var newrepin = '<li><a class="post-repins-avatar">' + obj_ipin.avatar48 + '</a> <a href="' + obj_ipin.home_url + '/user/' + obj_ipin.ul + '/">' + obj_ipin.ui + '</a> onto <strong>' + $('#repinform #board option:selected').text() + '</strong></li>';
	...
	$(document).on('submit', '#commentform', function() {
		...
		$('.form-submit').append(' <img class="comment-status-ajax-loader" src="' + obj_ipin.stylesheet_directory_uri + '/img/ajax-loader-2.gif" />');
		...
		$.ajax({
			...
			success: function() {
				...
				var newcomment = '<li><div class="comment-avatar">' + obj_ipin.avatar48 + '</div><div class="comment-content"><strong><a href="' + obj_ipin.home_url + '/user/' + obj_ipin.ul + '/">' + obj_ipin.ui + '</a></strong> / ' + obj_ipin.current_date + '<p>' + commenttext + '</p></div></li>';
	...
	//functions from this point mainly for frontpage only
	...
	$masonry.on('submit', '.masonry-meta form', function() {
		...
		$('#commentform-' + post_id + ' .form-submit').append(' <img class="comment-status-ajax-loader" src="' + obj_ipin.stylesheet_directory_uri + '/img/ajax-loader-2.gif" />');
	...
	//lightbox
	$masonry.on('click', '.featured-thumb, .featured-thumb-video, .featured-thumb-gif', function() {
		if ($(document).width() > 590 && navigator.userAgent.indexOf('iPad') == -1) {
			var lightbox = $('#post-lightbox');
			var href = $(this).closest('a').attr('href');

			$('body').css('overflow', 'hidden');
			lightbox.html('<div class="post-wrapper text-align-center"><p></p><p><img src="' + obj_ipin.stylesheet_directory_uri + '/img/ajax-loader-2.gif" /></p></div>')
	...
	function showRequest(formData, jqForm, options) {
		$('#pin-upload-from-web-wrapper, #bookmarklet').slideUp();
	...
	function showRequest_web(formData, jqForm, options) {
		...
		$('#pin-upload-from-computer-wrapper, #bookmarklet').slideUp();
		...
		if($.inArray(ext, ['gif','png','jpg','jpeg']) == -1) {
			$.get(obj_ipin.stylesheet_directory_uri + '/ipin_fetch.php?url=' + encodeURIComponent(input_url), function(data){
					...
					function display_thumbnails(imgarr, videoflag) {
						if (!imgarr.length) {
							...
							for (var i = 0; i < imgarr.length; i++) {
								if (videoflag == '0') {
									imgstr += '<div class="ipinimgwrapper" data-href="' + obj_ipin.home_url + '/story-settings/?m=bm&imgsrc=' + encodeURIComponent(imgarr[i][0].replace('http','')) + '&source=' + encodeURIComponent(input_url.replace('http','')) + '&title=' + page_title + '&video=' + videoflag + '"><div class="ipinbutton">+</div><img src="' + imgarr[i][0] + '" /></div>';
								} else {
									imgstr += '<div class="ipinimgwrapper" data-href="' + obj_ipin.home_url + '/story-settings/?m=bm&imgsrc=' + encodeURIComponent(imgarr[i][0].replace('http','')) + '&source=' + encodeURIComponent(input_url.replace('http','')) + '&title=' + page_title + '&video=' + videoflag + '"><div class="ipinbutton">+</div><div class="ipindimension">' + obj_ipin.__Video + '</div><img src="' + imgarr[i][0] + '" /></div>';
								}
							}
	...
	$('#pin-postdata-form').submit(function() {
		...
		// ES postdata custom hook
                $('.es_hook_postdata').each(function() {
	...
	$('#avatar-delete').on('mouseup', function() { 
		var ajaxloader = $('.ajax-loader-delete-avatar');
		var delete_btn = $(this);
